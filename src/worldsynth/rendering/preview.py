...
def render_preview(
    ...
    *,
    ...
    if "zones" in enabled:
        colors = {"encounter": (139, 61, 180, 82), "safe": (38, 190, 126, 70), "secret": (245, 194, 66, 85), "narrative": (78, 132, 220, 75)}
        for zone in compiled.zones:
            draw.rectangle(shifted(_world_rect(zone.rect, scale)), fill=colors[zone.kind], outline=colors[zone.kind][0:3] + (210,), width=2)
    if "objects" in enabled:
        for item in compiled.decorative_layers + compiled.objects:
            ...
            if not item.generated and scale >= 10:
                draw.text((rect[0] + 1, rect[1] + 1), item.id[:8], fill="#ffffffdd", font=ImageFont.load_default())
    ...
    if "transitions" in enabled:
        for transition in compiled.transitions:
            draw.rectangle(shifted(_world_rect(transition.rect, scale)), outline=(50, 225, 235, 255), width=3)
    if "spawns" in enabled:
        for spawn in compiled.spawns:
            ...
            draw.ellipse((cx - 4, cy - 4, cx + 4, cy + 4), fill=(50, 125, 255, 255), outline="white")
    ...
    if "edges" in enabled:
        for edge in compiled.edge_contracts:
            if edge.side in ("north", "south"):
                ...
                coords = (x, header + edge.position * scale, x + 3, header + (edge.position + edge.width) * scale)
            draw.rectangle(coords, fill=(255, 62, 210, 255))
    draw.rectangle((0, 0, image.width, header - 1), fill="#11151b")
    draw.text((7, 7), f"{compiled.display_name}  [{compiled.map_id}] seed={compiled.seed} hash={compiled.canonical_hash[:10]}", fill="white", font=ImageFont.load_default())
